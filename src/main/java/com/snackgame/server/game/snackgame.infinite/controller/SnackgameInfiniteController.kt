...
import com.snackgame.server.game.snackgame.service.SnackgameInfiniteService
...
게임 보드는 추후 제공 예정"""
    ...
    @PostMapping
    fun startSessionFor(@Authenticated member: Member): SnackgameInfiniteResponse =
...
현재는 점수 수정만 가능하며, 기존 점수가 덮어쓰기된다."""
    ...
    fun update(
        ...
        @RequestBody request: @Valid SnackgameInfiniteUpdateRequest,
    ): SnackgameInfiniteResponse = snackgameService.update(member.id, sessionId, request)
...
일시정지된 세션은 별도로 종료하지 않아도 **7일** 후 자동으로 만료된다."""
    ...
    fun pause(@Authenticated member: Member, @PathVariable sessionId: Long): SnackgameInfiniteResponse =
        snackgameService.pause(member.id, sessionId)

    @Operation(summary = "스낵게임 무한모드 세션 재개", description = "해당 세션을 재개한다")
    @PostMapping("/{sessionId}/resume")
    fun resume(@Authenticated member: Member, @PathVariable sessionId: Long): SnackgameInfiniteResponse =
        snackgameService.resume(member.id, sessionId)

    @Operation(summary = "스낵게임 무한모드 세션 종료", description = "세션을 종료한다")
    @PostMapping("/{sessionId}/end")
    fun end(@Authenticated member: Member, @PathVariable sessionId: Long): SnackgameInfiniteEndResponse =
